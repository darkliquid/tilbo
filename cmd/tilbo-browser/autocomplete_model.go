...
func NewAutocompleteModel(parent *qt6.QObject, dc *DaemonClient, ch chan<- func()) *AutocompleteModel {
	m := &AutocompleteModel{
		...
		roleNamesMap: map[int][]byte{
			...
			AcTriggerRole: []byte("acTrigger"), // Used to manually trigger search from QML (QModelIndex(0) / dummy item)
		},
	}
	...
	m.OnSetData(func(super func(*qt6.QModelIndex, *qt6.QVariant, int) bool, index *qt6.QModelIndex, value *qt6.QVariant, role int) bool {
...
func (m *AutocompleteModel) fetchAutocomplete(prefix string) {
	...
	m.daemonClient.ListTagsAsync(context.Background(), req, m.mainThreadCh, func(resp *ipcv1.ListTagsResponse, err error) {
		m.Clear()
		m.SetItemRoleNames(m.roleNamesMap)
		if err == nil && resp != nil {
