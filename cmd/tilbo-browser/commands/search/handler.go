...
func Handle(ctx context.Context, cmd core.Command, hctx core.HandlerContext) error {
	...
	hctx.Mutate(func(st *core.State) {
	...
	opCtx, cancel := hctx.OpContext(s.OperationID(), core.DefaultOperationTimeout)
	go func() {
		defer cancel()
		...
		if daemonFiles != nil || daemonErr != nil {
			files = core.EnsureSearchSource(daemonFiles, s.Chips, localFiles)
			err = core.BuildSearchError(daemonErr, localErr)
			if len(files) > 0 {
				err = nil
			}
		}
		...
		if err != nil {
			...
			hctx.Publish(ctx, core.OperationFailedEvent{
				EventBase: core.EventBase{OpID: s.OperationID(), At: time.Now()},
				Command:   s.Type(),
				Err:       err,
			})
			return
		...
		hctx.Mutate(func(st *core.State) {
			st.SearchResults = append([]core.SearchFile(nil), files...)
			st.LastSearch = time.Now()
		})

		hctx.Publish(ctx, core.SearchCompletedEvent{
			EventBase: core.EventBase{OpID: s.OperationID(), At: time.Now()},
			Chips:     append([]string(nil), s.Chips...),
			Files:     append([]core.SearchFile(nil), files...),
		})
	}()
