...
func TestLoadDirectoryFiltersHiddenFiles(t *testing.T) {
	...
	entries, hydratePaths, err := state.LoadDirectory(root, false)
	if err != nil {
...
func TestLoadDirectoryIncludesHiddenWhenEnabled(t *testing.T) {
	...
	entries, hydratePaths, err := state.LoadDirectory(root, true)
	if err != nil {
		t.Fatalf("load directory: %v", err)
	}
	...
	entryByPath := make(map[string]core.DirectoryEntry, len(entries))
