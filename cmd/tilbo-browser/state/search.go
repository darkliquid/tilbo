...
func LocalSearch(chips []string, limit uint32, allowHidden bool) ([]core.SearchFile, error) {
	if limit == 0 {
	...
	if len(globPatterns) == 0 {
		return []core.SearchFile{}, nil
	}

	maxResults := maxResultsFromLimit(limit)
	seen := make(map[string]struct{})
	files := make([]core.SearchFile, 0, maxResults)
...
func appendSearchMatches(
	...
	for _, match := range matches {
		...
		files = append(files, core.SearchFile{
			Path:  match,
			Tags:  []string{},
			Size:  info.Size(),
...
func FilterSearchFilesByHidden(files []core.SearchFile, allowHidden bool) []core.SearchFile {
	if allowHidden {
