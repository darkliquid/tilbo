...
func NewDirectoryProjection() (core.EventSubscriber, *ProjectionState) {
	ps := NewProjectionState(State{DirectoryEntries: []core.DirectoryEntry{}})
	sub := func(_ context.Context, evt core.Event) {
		...
		ps.mutate(func(s *State) {
			...
			s.DirectoryEntries = append([]core.DirectoryEntry(nil), e.Entries...)
...
func NewSearchProjection() (core.EventSubscriber, *ProjectionState) {
	ps := NewProjectionState(State{SearchResults: []core.SearchFile{}, SearchChips: []string{}})
	sub := func(_ context.Context, evt core.Event) {
		...
		ps.mutate(func(s *State) {
			...
			s.SearchResults = append([]core.SearchFile(nil), e.Files...)
...
func NewAutocompleteProjection() (core.EventSubscriber, *ProjectionState) {
...
func NewPlacesProjection() (core.EventSubscriber, *ProjectionState) {
	ps := NewProjectionState(State{Places: []core.PlaceEntry{}})
	sub := func(_ context.Context, evt core.Event) {
		e, ok := evt.(core.PlacesRefreshedEvent)
		if !ok {
			return
		}

		ps.mutate(func(s *State) {
			s.Places = append([]core.PlaceEntry(nil), e.Places...)
...
func NewPortalProjection() (core.EventSubscriber, *ProjectionState) {
	ps := NewProjectionState(State{WindowMode: "browser"})
	sub := func(_ context.Context, evt core.Event) {
		switch e := evt.(type) {
		case core.PortalOpenedEvent:
			ps.mutate(func(s *State) {
				s.WindowMode = e.Mode
				s.PortalSelection = []string{}
			})
		case core.PortalClosedEvent:
			ps.mutate(func(s *State) {
...
func NewFailureProjection() (core.EventSubscriber, *ProjectionState) {
	ps := NewProjectionState(State{})
	sub := func(_ context.Context, evt core.Event) {
		e, ok := evt.(core.OperationFailedEvent)
		if !ok {
			return
		}
...
type ProjectionSubscribers struct {
	Directory core.EventSubscriber
	Search    core.EventSubscriber
	Auto      core.EventSubscriber
	Places    core.EventSubscriber
	Portal    core.EventSubscriber
	Failures  core.EventSubscriber
}
...
func cloneState(s State) State {
	...
	out.DirectoryEntries = append([]core.DirectoryEntry(nil), s.DirectoryEntries...)
	out.SearchResults = append([]core.SearchFile(nil), s.SearchResults...)
	out.Autocomplete = append([]string(nil), s.Autocomplete...)
	out.Places = append([]core.PlaceEntry(nil), s.Places...)
