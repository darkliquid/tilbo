...
func BuildPlaces() ([]core.PlaceEntry, error) {
	...
	places := make([]core.PlaceEntry, 0, defaultPlacesCapacity)
	for _, p := range []core.PlaceEntry{
		{Name: "Home", Path: home},
	...
	if fuseActive {
		places = append(places,
			core.PlaceEntry{Name: "@recent", Path: fuseMount + "/@recent"},
			core.PlaceEntry{Name: "@untagged", Path: fuseMount + "/@untagged"},
			core.PlaceEntry{Name: "@browse", Path: fuseMount + "/@browse"},
		)
	}
