...
func TestLocalSearchUsesGlobChips(t *testing.T) {
	...
	files, err := state.LocalSearch([]string{"glob:" + filepath.Join(root, "*.txt")}, 10, false)
	if err != nil {
...
func TestFilterSearchFilesByHidden(t *testing.T) {
	...
	filtered := state.FilterSearchFilesByHidden(files, false)
	if len(filtered) != 1 || filtered[0].Path != "/tmp/visible.txt" {
		t.Fatalf("unexpected filtered files: %#v", filtered)
	}

	all := state.FilterSearchFilesByHidden(files, true)
	if len(all) != 2 {
