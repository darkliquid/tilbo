...
func (s *stubMethods) StatFile(string) (browser.FileStat, error) {
	return browser.FileStat{}, fmt.Errorf("not implemented")
}
...
func (s *stubMethods) ModifyTags([]string, []string, string) (browser.TagResult, error) {
	return browser.TagResult{}, fmt.Errorf("not implemented")
}
...
func startServer(t *testing.T, m browser.Methods) (sockPath string, srv *Server, cancel context.CancelFunc) {
	t.Helper()
	dir := t.TempDir()
...
func readResponse(t *testing.T, scanner *bufio.Scanner) map[string]interface{} {
	t.Helper()
	if !scanner.Scan() {
...
func TestBroadcast_DeliveredToAllClients(t *testing.T) {
	...
	for i, c := range clients {
		...
		args, ok := resp["args"].(map[string]interface{})
		if !ok {
...
func TestBroadcast_IndexUpdated(t *testing.T) {
	...
	args := resp["args"].(map[string]interface{})
...
func TestNewlineDelimitedFraming(t *testing.T) {
	counter := 0
	m := &stubMethods{
		listTagsFn: func(string) ([]string, error) {
			counter++
			return []string{fmt.Sprintf("tag%d", counter)}, nil
		},
	}
	sockPath, _, cancel := startServer(t, m)
	defer cancel()
